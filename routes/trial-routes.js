module.exports = function (app, rethinkdb) {
  ...
  function listTrials(req, res, next) {
      rethinkdb.table('trials').orderBy({index: "createdAt"}).run(rethinkdb.conn, function(error, cursor) {
          if (error) {
  ...
  function getTrial(req, res, next) {
    ...
    rethinkdb.table('trials').get(trialID).run(rethinkdb.conn, function(error, result) {
      if(error) {
        handleError(res, error) 
        next();
      }
  ...
  function createTrial(req, res, next) {
      ...
      rethinkdb.table('trials').insert(trial, {returnChanges: true}).run(rethinkdb.conn, function(error, result) {
          if (error) {
              handleError(res, error) 
              next();
          }
  ...
  function updateTrial(req, res, next) {
    ...
    rethinkdb.table('trials').get(trialID).run(rethinkdb.conn, function(error, result) {
      if(error) {
        ...
        rethinkdb.table('trials').get(trialID).update(trial, {returnChanges: true}).run(rethinkdb.conn, function(error, result) {
          if(error) {
            //handleError(res, error) 
            throw error;
            next();
  ...
  function deleteTrial(req, res, next) {
    ...
    rethinkdb.table('trials').get(trialID).delete().run(rethinkdb.conn, function(error, result) {
      if(error) {
        handleError(res, error) 
        next();
      }
